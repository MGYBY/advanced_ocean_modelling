...
! initial distribution of Eulerian tracer
DO i = 16,nz+1
DO k = 0,nx+1
 c(i,k) = 1.0
 cn(i,k) = 1.0
...
! random initialisation of float locations
...
DO k = 0,nx+1
 rho(0,k) = 0.0
 DO i = 1,nz+1
  rho(i,k) = rho(i-1,k) + RHOREF*N2*dz/G
 END DO
END DO

DO k = 0,nx+1
DO i = 1,nz+1
  rho(i,k) = rho(i,k)+ran3(ist)*0.0001
END DO
END DO
...
drho = dt*2.5e-4*QQ/(CP*DZ)

DO k = 1,nx
  rho(1,k) = rho(1,k) + drho
  rho(0,k) = rho(1,k)
END DO
