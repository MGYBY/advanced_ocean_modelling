...
f = 1.e-4
alpha = dt*f
r = 0.e-3 ! disable bottom friction
...
!    IF(i == ibv(j,k)) um = 0.25*(u(ibu(j,k),j,k)+u(ibu(j,k-1),j,k-1)+ u(ibu(j+1,k),j+1,k)+u(ibu(j+1,k-1),j+1,k-1))
    vm = 0.25*(v(i,j,k)+v(i,j,k+1)+v(i,j-1,k)+v(i,j-1,k+1))
!    IF(i == ibu(j,k)) vm = 0.25*(v(ibv(j,k),j,k)+v(ibv(j,k+1),j,k+1)+ v(ibv(j-1,k),j-1,k)+v(ibv(j-1,k+1),j-1,k+1))
