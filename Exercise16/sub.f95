...
! diffusion of u
 ...
 IF(.NOT.WET(i+1,k)) THEN
   speed = SQRT(u(i,k)*u(i,k)+vm*vm)
   dif2 = r*u(i,k)*speed
 END IF
...
! diffusion of v
 ...
 difh = ah*(dif1-dif2)/dx
 dif1 = 0.0
 IF(WET(i-1,k)) dif1 = az*(v(i-1,k)-v(i,k))/dz
 dif2 = az*(v(i,k)-v(i+1,k))/dz
 IF(.NOT.WET(i+1,k)) THEN
   speed = SQRT(v(i,k)*v(i,k)+um*um)
   dif2 = r*v(i,k)*speed
 END IF
