...
f = 1.e-4
alpha = dt*f
r = 0.e-3 ! bottom friction disabled

OPEN(50,file ='topo.dat',form='formatted')
DO j = 0,ny+1
  READ(50,'(102F12.6)')(depth(j,k),k=0,nx+1)
END DO
CLOSE(50)
...
! initial distribution of Eulerian concentration field
DO k = 0,nx+1
DO j = 21,ny+1
DO i = 0,nz+1
  IF(wet(i,j,k))c(i,j,k) = 1.0
END DO
END DO
END DO
...
DO i = 0,nz+1  
  IF(wet(i,j,1))THEN
    u(i,j,1) = u(i,j,1) + ad*(0.1-u(i,j,1))
    u(i,j,0) = u(i,j,1)
    v(i,j,0) = 0.0
  END IF
END DO
...
DO i = 0,nz+1  
  IF(wet(i,1,k))THEN
    v(i,1,k) = v(i,1,k) + ad*(0.1-v(i,1,k))
    v(i,0,k) = v(i,1,k)
    u(i,0,k) = 0.0 
  END IF
END DO
...
DO j = 1,ny
DO k = 1,nx
  aztop = 0.5*(az(1,j,k)+az(0,j,k))
  v(0,j,k) = v(1,j,k)
  u(0,j,k) = u(1,j,k) !+ad*dz*taux/(RHOREF*aztop)
END DO
END DO
...
! diffusion of v
 ...
 IF(dry(i,j,k+1)) dif1 = 0.0
 IF(depth(j,k+1)<=0.0) dif1 = -2.0*ahe*v(i,j,k)/dy ! no-slip condition

 ahw = 0.25*(ah(i,j,k)+ah(i,j+1,k)+ah(i,j,k-1)+ah(i,j+1,k-1))
 dif2 = ahw*(v(i,j,k)-v(i,j,k-1))/dx
 IF(dry(i,j,k-1)) dif2 = 0.0
 IF(depth(j,k-1)<=0.0) dif2 = 2.0*ahw*v(i,j,k)/dy ! no-slip condition
