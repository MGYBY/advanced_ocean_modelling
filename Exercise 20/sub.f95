...
DO k = 0,nx+1
  ...
  DO i = nb+1,nz+1
   wet(i,j,k) = .false.
  END DO
! wet(0,j,k) = .false. ! rigid-lid approximation disabled
END DO
END DO
...
DO j = 1,ny
  um = u(ib(j,k),j,k)
  vm = 0.25*(v(ib(j,k),j,k)+v(ib(j,k+1),j,k+1)+ &
& v(ib(j-1,k),j-1,k)+v(ib(j-1,k+1),j-1,k+1))
  speed = SQRT(um*um+vm*vm)
  tbu(j,k) = r*um*speed
  vm = v(ib(j,k),j,k)
  um = 0.25*(u(ib(j,k),j,k)+u(ib(j,k-1),j,k-1)+ &
& u(ib(j+1,k),j+1,k)+u(ib(j+1,k-1),j+1,k-1))  
  speed = SQRT(um*um+vm*vm)
  tbv(j,k) = r*vm*speed
END DO
END DO
