...
beta = 2.2e-11
DO j = 0,ny+1
  f(j) = beta*REAL(j-51)*dy
END DO

r = 0.e-3 ! friction disabled
...
DO j = 0,ny+1
DO k = 0,nx+1
 IF(depth(j,k) == 0.0) land(j,k) = .TRUE. 
END DO
END DO
...
! disable rigid-lid approximation
!wet(0,j,k) = .false.
...
DO j = 0,ny+1
DO k = 0,nx+1
  IF(wet(i,j,k).AND.dry(i+1,j,k))ib(j,k) = i
END DO
END DO
END DO
...
call eddyZ ! vertical turbulence closure scheme
call eddyH ! horizontal turbulence closure scheme
...
DO i = 0,nz+1
DO k = 0,nx+1
 rhon(i,0,k) = rhon(i,ny,k)
 rhon(i,ny+1,k) = rhon(i,1,k)
END DO
END DO
...
DO i = 1,nz
 ...
 div = dt*B(i,j,k)*(div1+div2+div3)
 khe = 0.5*(ah(i,j,k)+ah(i,j,k+1))
 dif1 = 100.*khe*(c(i,j,k+1)-c(i,j,k))/dx
 IF(dry(i,j,k+1)) dif1 = 0.0
 khw = 0.5*(ah(i,j,k)+ah(i,j,k-1))
 dif2 = 100.*khw*(c(i,j,k)-c(i,j,k-1))/dx
 IF(dry(i,j,k-1)) dif2 = 0.0
 khn = 0.5*(ah(i,j,k)+ah(i,j+1,k))
 dif3 = 100.*khn*(c(i,j+1,k)-c(i,j,k))/dy
 IF(dry(i,j+1,k)) dif3 = 0.0
 khs = 0.5*(ah(i,j,k)+ah(i,j-1,k))
 dif4 = 100.*khs*(c(i,j,k)-c(i,j-1,k))/dy
 IF(dry(i,j-1,k)) dif4 = 0.0
...
DO j = 1,ny
DO k = 1,nx
  azt = 0.25*(az(i,j,k)+az(i,j,k+1)+az(i-1,j,k)+az(i-1,j,k+1))
  u(0,j,k) = u(1,j,k) + ad*dz*taux(j,k)/(RHOREF*azt)
  azt = 0.25*(az(i,j,k)+az(i,j+1,k)+az(i-1,j,k)+az(i-1,j+1,k))
  v(0,j,k) = v(1,j,k) + ad*dz*tauy(j,k)/(RHOREF*azt)
END DO
END DO
...
! diffusion of w
 ...
 diffw = dt*(difh+difz)  
  ...
  IF(wet(i,j,k))THEN
    ...
    fm = f(j)
    alpha = fm*dt
    IF(wet(i,j,k+1)) ustar(i,j,k) = cos(alpha)*u(i,j,k) + sin(alpha)*vm + dt*pressx + advx(i,j,k) + diffu
    pressy = -drdy*(q(i,j+1,k)-q(i,j,k))-drdy*(p(i,j+1,k)-p(i,j,k))
    fm = 0.5*(f(j)+f(j+1))
    alpha = fm*dt
    IF(wet(i,j+1,k)) vstar(i,j,k) = cos(alpha)*v(i,j,k) - sin(alpha)*um + dt*pressy + advy(i,j,k) + diffv
    pressz = -drdz*(q(i-1,j,k)-q(i,j,k))
    IF(wet(i-1,j,k)) wstar(i,j,k) = w(i,j,k) + dt*pressz + advz(i,j,k) + diffw
  END IF
...
DO j = 1,ny
DO k = 1,nx
 dq(0,j,k) = -dt*RHOREF*G*( (usum(j,k)-usum(j,k-1) )/dx + ( vsum(j,k)-vsum(j-1,k) )/dy )
END DO
END DO
...
term1 = (u(i,j,k)-u(i,j,k-1))/dx
term1 = term1*term1

term2 = (v(i,j,k)-v(i,j-1,k))/dy
term2 = term2*term2

uN = 0.25*(u(i,j,k)+u(i,j,k-1)+u(i,j+1,k)+u(i,j+1,k-1))
