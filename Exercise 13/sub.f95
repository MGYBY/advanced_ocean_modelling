...
N2 = 0.0 ! stability frequency squared
...
DO k = 0,nx+1
  P(0,k) = 0.0
  DO i = 1,nz+1
    P(i,k) = P(i-1,k) + 0.5*(rho(i-1,k)+rho(i,k))*G*dz*COS(alpha)
  END DO
END DO
...
! diffusion of u
 ...
 dif1 = 0.0
 IF(WET(i-1,k)) dif1 = az*(u(i-1,k)-u(i,k))/dz
 dif2 = az*(u(i,k)-u(i+1,k))/dz
 difz = (dif1-dif2)/dz
 diffu = dt*(difh+difz)  
...
! diffusion of w
 ...
 diffw = dt*(difh+difz)  
  ...
  IF(wet(i,k))THEN
    ...
    pressx = pressx + G*0.5*(rho(i,k+1)+rho(i,k))/RHOREF*SIN(alpha)
