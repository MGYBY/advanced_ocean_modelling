...
! evaporation (5 cm per day converted to m/s)
DO k = 1,55
 evap(k) = 0.05/(24.*3600.)
END DO

! vanishing evaporation over adjacent ocean
DO k = 56,65
 evap(k) = 0.05/(24.*3600.)*(1.0-REAL(k-55)/10.)
END DO
...
REAL :: w1,w2, pgradx
REAL :: we, ww, wc, roc, coc
INTEGER :: idep

!---------------------
! evaporative forcing
IF(time<10.*24.*3600)then

! surface density flux
DO k = 1,nx
  rho(1,k) = rho(1,k)+dt*(RHOREF*betaS0+rho(1,k))*evap(k)/dz
  rho(0,k) = rho(1,k)
END DO

! surface evaporative water loss
DO k = 1,nx
  q(0,k) = q(0,k)-dt*evap(k)*RHOREF*G
END DO
...
! tidal forcing at ocean boundary (disabled)
!q(0,1) = -ad*0.25*SIN(2.*PI*(time+1800.)/period)*RHOREF*G
q(0,0) = q(0,1)
...
DO k = 1,nx
  ...
  IF(dummy>etaMAX(k))etaMAX(k) = dummy
...
! Gradient Richardson number
  ...
  Ri = max(Ri,0.0) ! convection is treated explicitly
  ff = 1.0+bb*Ri
  wurz = nu0/(ff**nn)+nub
!****** final value ******
  az(i,k) = wurz
